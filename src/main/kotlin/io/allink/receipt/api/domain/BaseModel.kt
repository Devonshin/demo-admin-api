...
data class PagedResult<T>(
  @param:Schema(title = "조회 데이터")
  val items: List<T>,
  @param:Schema(title = "전체 건수")
  val totalCount: Int,
  @param:Schema(title = "현재 페이지 번호")
  val currentPage: Int,
  @param:Schema(title = "전체 페이지 번호")
  val totalPages: Int
)
...
data class Sorter(
  @param:Schema(title = "정렬 대상", description = "정렬에 사용하고자 하는 필드명", example = "id")
  val field: String,
  @param:Schema(title = "정렬 순서", description = "ASC, DESC", allowableValues = ["ASC", "DESC"], example = "ASC")
...
data class Page(
  @param:Schema(title = "페이지 번호", description = "1 에서 무한대", example = "15", requiredMode = RequiredMode.REQUIRED)
  val page: Int = 1,
  @param:Schema(title = "페이지 길이", description = "최대 값 100", example = "10", requiredMode = RequiredMode.REQUIRED)
  val pageSize: Int = 10
)
