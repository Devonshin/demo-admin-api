...
@Serializable @Schema(title = "가맹점 결제 정보")
data class StoreBillingModel(
  @param:Schema(title = "결제 순번", description = "가맹점 등록 시 생성되는 결제 순번")
  override var id: Long? = null,
  @param:Schema(title = "가맹점 고유아이디", description = "가맹점 고유아이디")
  val storeUid: String,
  @param:Schema(title = "가맹점 서비스 등록 순번",
    description = "가맹점 서비스 등록 순번")
  val storeServiceSeq: Int,
  @param:Schema(title = "결제 토큰 고유아이디", description = "결제 토큰 고유아이디")
  val tokenUuid: @Contextual UUID,
  @param:Schema(title = "결제 진행 상태", description = "결제 진행 상태", defaultValue = "PENDING", example = "PENDING|COMPLETE|FAIL")
  val status: BillingStatusCode? = BillingStatusCode.PENDING,
  @param:Schema(title = "오늘 결제 금액", description = "오늘 결제 총액")
  val billingAmount: Int? = 0,
  @param:Schema(title = "환불 계좌 은행 코드", description = "환불 계좌 은행 코드, 은행 코드 목록 조회에서 확인 가능")
  val bankCode: String? = null,
  @param:Schema(title = "환불 계좌번호", description = "환불 계좌번호")
  val bankAccountNo: String? = null,
  @param:Schema(title = "계좌 예금주", description = "계좌 예금주")
  val bankAccountName: String? = null,
  @param:Schema(title = "등록일시", description = "등록일시", example = "2025-04-17 12:00:00.213123")
  val regDate: @Contextual LocalDateTime,
  @param:Schema(title = "등록인 아이디", description = "등록인 아이디")
  val regBy: @Contextual UUID,
) : BaseModel<Long>

@Serializable @Schema(title = "가맹점 결제 정보 등록")
