...
data class MerchantGroupModel(
  ...
  val remoteIps: String,
  @Schema(title = "서비스 시작일시", description = "서비스 시작일시", nullable = false, requiredMode = RequiredMode.REQUIRED, example = "2025-04-17 12:00:00.213123")
  val serviceStartAt: @Contextual LocalDateTime,
  @Schema(title = "서비스 종료일시", description = "서비스 종료일시", nullable = false, requiredMode = RequiredMode.REQUIRED, example = "2025-04-17 12:00:00.213123")
  val serviceEndAt: @Contextual LocalDateTime,
  @Schema(title = "등록 상태", description = "등록 상태 - 계약 내용에 따라", nullable = false, requiredMode = RequiredMode.REQUIRED)
  val status: MerchantGroupStatus,
  @Schema(title = "영수증 너비 인치값", description = "포스 프린트 단말기의 영수증 너비 인치값", nullable = false, requiredMode = RequiredMode.REQUIRED)
  val receiptWidth: String,
  @Schema(title = "권한 목록", description = "올링크 서비스에서 허용한 권한 목록, 콤마 구분자", nullable = false, requiredMode = RequiredMode.REQUIRED)
  val authorities: String,
  @Schema(title = "영수증 데이터 타입", description = "포스 에이전트가 영수증 데이터를 취급하는 방법", example = "MERCHANT_RECEIPT|MERCHANT_PARSING|AGENT_HOOKING|AGENT_FILE")
